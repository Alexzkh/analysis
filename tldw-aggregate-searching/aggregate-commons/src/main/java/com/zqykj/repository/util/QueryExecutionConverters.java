...
import com.zqykj.util.NullableWrapper;
...
public class QueryExecutionConverters {
    ...
    static {
        ...
        WRAPPER_TYPES.add(WrapperType.singleValue(Optional.class));
        UNWRAPPER_TYPES.add(WrapperType.singleValue(Optional.class));

        WRAPPER_TYPES.add(NullableWrapperToCompletableFutureConverter.getWrapperType());

        if (VAVR_PRESENT) {

            // TODO 需要 增加 Java 8+ 函数式库Vavr封装的Utils
...
//            ALLOWED_PAGEABLE_TYPES.add(io.vavr.collection.Seq.class);
    ...
    /**
     * A Spring {@link Converter} to support JDK 8's {@link Optional}.
     *
