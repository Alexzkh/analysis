...
public class UnadjustedAccountAggBuilder extends FundTacticsCommonAggBuilder implements UnadjustedAccountAggParamFactory {
    ...
    @Override
    public AggregationParams unadjustedAccountAnalysis(UnadjustedAccountAnalysisRequest request, int groupSize) {
        ...
        if (sortRequest.getProperty().equals("linkedAccountTimes")) {
            AggregationParams linkedAccountTimes = AggregationParamsBuilders.cardinality("linkedAccountTimes", FundTacticsAnalysisField.TRANSACTION_OPPOSITE_CARD);
            queryCardGroup.setPerSubAggregation(linkedAccountTimes);
        }
        // 交易总次数
        AggregationParams tradeTotalTimes = AggregationParamsBuilders.count("tradeTotalTimes", FundTacticsAnalysisField.QUERY_CARD, null);
        queryCardGroup.setPerSubAggregation(tradeTotalTimes);
        // 交易净和
        AggregationParams tradeNetSum = AggregationParamsBuilders.sum("tradeNetSum", FundTacticsAnalysisField.TRANSACTION_MONEY);
        ...
        PipelineAggregationParams sort = fundTacticsPartUniversalAggSort(request.getSortRequest(), offset, pageRequest.getPageSize());
    ...
    public AggregationParams unadjustedAccountAnalysisSecondQuery(UnadjustedAccountAnalysisRequest request, int groupSize, String... showFields) {
        ...
        PipelineAggregationParams sort = fundTacticsPartUniversalAggSort(0, request.getPageRequest().getPageSize());
        queryCardGroup.setPerSubAggregation(sort);
        return queryCardGroup;
    }
    ...
    private void computeFeatureRatio(UnadjustedAccountAnalysisRequest request, AggregationParams queryCardGroup) {
        // 交易总金额
        AggregationParams tradeTotalAmount = AggregationParamsBuilders.sum("tradeTotalAmount", FundTacticsAnalysisField.CHANGE_MONEY);
        ...
        AggregationParams creditSum = AggregationParamsBuilders.sum("creditSum", FundTacticsAnalysisField.CHANGE_MONEY);
        ...
        AggregationParams payoutSum = AggregationParamsBuilders.sum("payoutSum", FundTacticsAnalysisField.CHANGE_MONEY);
        ...
        bucketPath.put("creditAmountTotal", "filterCredit>creditSum");
        bucketPath.put("payoutAmountTotal", "filterPayout>payoutSum");
        bucketPath.put("tradeAmountTotal", "tradeTotalAmount");
